...
{
    public string Root { get; } = Path.Combine(Path.GetTempPath(), "honeycomb-tests", Guid.NewGuid().ToString("N"));
...
public class SongCompileLogicTests : IDisposable
{
    ...
    public SongCompileLogicTests()
    {
        ...
        var compile = new SongCompileService(_settings, _notifications,
            new Honeycomb.Infrastructure.ExternalProcessService(_platform), _platform,
