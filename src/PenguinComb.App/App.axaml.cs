...
using PenguinComb.App.Services;
...
public partial class App : Avalonia.Application
{
    ...
    private async Task StartupAsync(IClassicDesktopStyleApplicationLifetime desktop)
    {
        try
        {
            ...
            var settings = Services.GetRequiredService<PenguinComb.Application.Abstractions.ISettingsService>();
        ...
        catch (Exception ex)
        {
            ...
            {
                Text = $"PenguinComb failed to start:\n\n{ex.Message}",
                TextWrapping = Avalonia.Media.TextWrapping.Wrap,
                Margin = new Avalonia.Thickness(16)
            };
            desktop.MainWindow = new Avalonia.Controls.Window
            {
                Title = "PenguinComb - Startup Error",
                Content = error,
                Width = 480,
                Height = 220
            };
    ...
    private void ConfigureLogging()
    {
        try
        {
            ...
            Console.WriteLine($"PenguinComb {typeof(App).Assembly.GetName().Version} - created by Kelvin Klein.");
        }
        catch (Exception ex)
        {
